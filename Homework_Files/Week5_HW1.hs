...
mkPolicy pkh deadline ctx =
    traceIfFalse "SIGNATURE IS MISSING" checkSig      && --txSignedBy allows us to check if a transaction was signed by the given public key.
    traceIfFalse "DEADLINE HAS PAST"    checkDeadline    --This only works in a single public key contract. To use multiple keys use txInfoSignatories
  where
    info :: TxInfo
...
policy pkh deadline = mkMonetaryPolicyScript $
    ...
    `PlutusTx.applyCode`                                -- It appears that `PlutusTx.applyCode` needs to be called per variable used in the policy
    PlutusTx.liftCode pkh                               -- Since we have pkh and deadline we will need to use this code twice
    `PlutusTx.applyCode`
    PlutusTx.liftCode deadline

curSymbol :: PubKeyHash -> Slot -> CurrencySymbol
curSymbol pkh deadline = scriptCurrencySymbol $ policy pkh deadline              -- YUP, still missing that Slot deats
...
mint mp = do
    ...
    if now > deadline
        then Contract.logError @String "deadline passed"
            ...
            let val     = Value.singleton (curSymbol pkh deadline) (mpTokenName mp) (mpAmount mp) -- singleton :: CurrencySymbol -> TokenName -> Integer -> Value
